...
test("缺欄位、錯誤網址與無替代文字圖片會被阻擋",()=>{const blockers=getPublishBlockers({...complete,slug:"中文",cover_image:"",content:'<img src="https://huayi.tw/a.jpg">'});assert.ok(blockers.some((item)=>item.includes("封面")));assert.ok(blockers.some((item)=>item.includes("網址")));assert.ok(blockers.some((item)=>item.includes("替代文字")));});
