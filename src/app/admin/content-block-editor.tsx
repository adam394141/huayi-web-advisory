...
type ImageBlock = { id: string; kind: "image"; src: string; alt: string; caption: string; width?: number; height?: number };
...
function parseContent(html: string): Block[] {
  ...
  for (const node of Array.from(doc.body.children)) {
    ...
    if (image) {
      ...
      blocks.push({ id: id(), kind: "image", src: image.getAttribute("src") || "", alt: image.getAttribute("alt") || "", caption,
        width: Number(image.getAttribute("width")) || undefined, height: Number(image.getAttribute("height")) || undefined });
...
function serialize(blocks: Block[]) {
  return blocks.map((block) => block.kind === "html" ? block.html :
    `<figure><img src="${escapeAttribute(block.src)}" alt="${escapeAttribute(block.alt)}"${block.width ? ` width="${block.width}"` : ""}${block.height ? ` height="${block.height}"` : ""}><figcaption>${escapeAttribute(block.caption)}</figcaption></figure>`).join("\n");
}
...
export function ContentBlockEditor({ value, onChange, collection, itemId, accessToken, previewHref }: {
  ...
  async function upload() {
    ...
    try {
      ...
      form.append("file", file); form.append("collection", collection); form.append("itemId", itemId);
      ...
      insert({ id: id(), kind: "image", src: result.url, alt: alt.trim() || "作品圖片", caption: caption.trim(), ...dimensions });
      setFile(null); setAlt(""); setCaption(""); setDimensions(null); if (fileInput.current) fileInput.current.value = "";
      setMessage("圖片已插入內文；請記得按下頁面底部的「儲存修改」。");
  ...
  return <section className="space-y-4 rounded-2xl border border-neutral-200 p-4">
    ...
    <div className="space-y-3">{blocks.map((block, index) => <div key={block.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); moveTo(Number(event.dataTransfer.getData("text/plain")), index); }} onClick={() => setSelected(index)} className={`rounded-2xl border p-3 ${selected === index ? "border-amber-500 bg-amber-50/40" : "border-neutral-200"}`}>
      ...
      {block.kind === "html" ? <div className="min-h-20 rounded-xl bg-white p-4 leading-relaxed outline-none ring-amber-400 focus:ring-2 [&_h2]:text-2xl [&_h2]:font-semibold [&_p]:mb-3" contentEditable suppressContentEditableWarning dangerouslySetInnerHTML={{ __html: block.html }} onPaste={(event) => { event.preventDefault(); document.execCommand("insertText", false, event.clipboardData.getData("text/plain")); }} onBlur={(event) => { const next = [...blocks]; next[index] = { ...block, html: event.currentTarget.innerHTML }; commit(next); }} /> : <div className="grid gap-4 md:grid-cols-[1.4fr_1fr]">
        ...
        <div className="space-y-3"><label className="block text-sm">替代文字<input className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2" value={block.alt} onChange={(event) => { const next=[...blocks]; next[index]={...block,alt:event.target.value}; commit(next); }} /></label><label className="block text-sm">圖片說明<input className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2" value={block.caption} onChange={(event) => { const next=[...blocks]; next[index]={...block,caption:event.target.value}; commit(next); }} /></label><p className="text-sm text-neutral-500">{block.width && block.height ? `${block.width} × ${block.height} px` : "尺寸未記錄"}・原比例顯示、不裁切</p></div>
      </div>}
    </div>)}</div>
    <div className="rounded-2xl bg-neutral-100 p-4"><h4 className="font-medium">上傳圖片並插入目前區塊下方</h4><div className="mt-3 grid gap-3 md:grid-cols-2"><input ref={fileInput} type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseFile} /><input className="rounded-xl border border-neutral-300 px-3 py-2" placeholder="替代文字（描述圖片內容）" value={alt} onChange={(event) => setAlt(event.target.value)} /><input className="rounded-xl border border-neutral-300 px-3 py-2" placeholder="圖片說明（可留白）" value={caption} onChange={(event) => setCaption(event.target.value)} /><button type="button" className="rounded-full bg-neutral-900 px-5 py-2 text-white disabled:opacity-40" disabled={!file || !dimensions || uploading} onClick={upload}>{uploading ? "上傳中…" : "上傳並插入"}</button></div>{dimensions && <p className="mt-3 text-sm">實際尺寸：{dimensions.width} × {dimensions.height} px。{dimensions.width < 1600 ? "內頁建議寬度至少 1600 px；此圖可上傳，但放大可能模糊。" : "符合內頁建議寬度。"}</p>}{message && <p role="status" className="mt-3 text-sm text-amber-800">{message}</p>}</div>
  </section>;
}
